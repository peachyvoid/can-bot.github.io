...
//#region node_modules/jsmediatags/build2/StringUtils.js
var require_StringUtils = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports = {
		readUTF16String: function readUTF16String(bytes, bigEndian, maxBytes) {
			...
			for (var j = 0; ix < maxBytes; j++) {
				...
				ix += 2;
				if (word1 == 0) break;
				else if (byte1 < 216 || byte1 >= 224) arr[j] = String.fromCharCode(word1);
				else {
		...
		readUTF8String: function readUTF8String(bytes, maxBytes) {
			var ix = 0;
			maxBytes = Math.min(maxBytes || bytes.length, bytes.length);
			if (bytes[0] == 239 && bytes[1] == 187 && bytes[2] == 191) ix = 3;
			var arr = [];
			for (var j = 0; ix < maxBytes; j++) {
				var byte1 = bytes[ix++];
				if (byte1 == 0) break;
				else if (byte1 < 128) arr[j] = String.fromCharCode(byte1);
...
//#endregion
//#region node_modules/jsmediatags/build2/MediaFileReader.js
var require_MediaFileReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _classCallCheck(instance, Constructor) {
		if (!(instance instanceof Constructor)) throw new TypeError("Cannot call a class as a function");
	}
	function _defineProperties(target, props) {
		...
		_createClass(MediaFileReader, [
			{
				...
				value: function init(callbacks) {
					var self = this;
					if (this._isInitialized) setTimeout(callbacks.onSuccess, 1);
			...
			},
			{
				...
				value: function getBytesAt(offset, length) {
					...
					for (var i = 0; i < length; i++) bytes[i] = this.getByteAt(offset + i);
					return bytes;
				}
			},
			{
				...
				value: function isBitSetAt(offset, bit) {
					return (this.getByteAt(offset) & 1 << bit) != 0;
				}
			},
			{
				key: "getSByteAt",
			...
			},
			{
				...
				value: function getStringAt(offset, length) {
					var string = [];
					for (var i = offset, j = 0; i < offset + length; i++, j++) string[j] = String.fromCharCode(this.getByteAt(i));
					return string.join("");
				}
			},
			{
			...
			},
			{
				...
				value: function getSynchsafeInteger32At(offset) {
					...
					return this.getByteAt(offset + 3) & 127 | (size3 & 127) << 7 | (size2 & 127) << 14 | (size1 & 127) << 21;
				}
			}
		], [{
			key: "canReadFile",
...
//#endregion
//#region node_modules/jsmediatags/build2/ChunkedFileData.js
/**
...
* 
*/
var require_ChunkedFileData = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _classCallCheck(instance, Constructor) {
		if (!(instance instanceof Constructor)) throw new TypeError("Cannot call a class as a function");
	}
	function _defineProperties(target, props) {
		...
		_createClass(ChunkedFileData, [
			{
				...
				value: function addData(offset, data) {
					...
					if (chunkRange.startIx === NOT_FOUND) this._fileData.splice(chunkRange.insertIx || 0, 0, {
						offset,
						data
					});
					else {
			...
			},
			{
				...
				value: function _concatData(dataA, dataB) {
					if (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(dataA)) {
						...
						return dataAandB;
					} else return dataA.concat(dataB);
				}
			},
			{
				key: "_sliceData",
				value: function _sliceData(data, begin, end) {
					if (data.slice) return data.slice(begin, end);
					else return data.subarray(begin, end);
				}
			},
			{
				key: "_getChunkRange",
				value: function _getChunkRange(offsetStart, offsetEnd) {
					...
					for (var i = 0; i < this._fileData.length; i++, insertIx = i) {
						...
						if (offsetEnd < chunkOffsetStart - 1) break;
						if (offsetStart <= chunkOffsetEnd + 1 && offsetEnd >= chunkOffsetStart - 1) {
							startChunkIx = i;
							break;
						}
					}
					if (startChunkIx === NOT_FOUND) return {
						startIx: NOT_FOUND,
						endIx: NOT_FOUND,
						insertIx
					};
					for (var i = startChunkIx; i < this._fileData.length; i++) {
						var chunkOffsetStart = this._fileData[i].offset;
						var chunkOffsetEnd = chunkOffsetStart + this._fileData[i].data.length;
						if (offsetEnd >= chunkOffsetStart - 1) endChunkIx = i;
						if (offsetEnd <= chunkOffsetEnd + 1) break;
					}
					if (endChunkIx === NOT_FOUND) endChunkIx = startChunkIx;
					return {
			...
			},
			{
				...
				value: function hasDataRange(offsetStart, offsetEnd) {
					for (var i = 0; i < this._fileData.length; i++) {
						...
						if (offsetEnd < chunk.offset) return false;
						if (offsetStart >= chunk.offset && offsetEnd < chunk.offset + chunk.data.length) return true;
					}
					return false;
				}
			},
			{
				...
				value: function getByteAt(offset) {
					...
					if (dataChunk) return dataChunk.data[offset - dataChunk.offset];
...
//#endregion
//#region node_modules/xhr2/lib/browser.js
var require_browser = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = XMLHttpRequest;
}));
//#endregion
//#region node_modules/jsmediatags/build2/XhrFileReader.js
var require_XhrFileReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		_createClass(XhrFileReader, [
			{
				key: "_init",
				value: function _init(callbacks) {
					if (XhrFileReader._config.avoidHeadRequests) this._fetchSizeWithGetRequest(callbacks);
					else this._fetchSizeWithHeadRequest(callbacks);
				}
			},
			{
					...
					this._makeXHRRequest("HEAD", null, {
						onSuccess: function onSuccess(xhr) {
							...
							if (contentLength) {
								...
								callbacks.onSuccess();
							} else self._fetchSizeWithGetRequest(callbacks);
						},
						onError: callbacks.onError
					});
				}
			...
			},
			{
				...
				value: function _parseContentLength(xhr) {
					...
					else return parseInt(contentLength, 10);
				}
			},
			{
				...
				value: function _parseContentRange(xhr) {
					...
					if (contentRange) {
						...
						if (!parsedContentRange) throw new Error("FIXME: Unknown Content-Range syntax: " + contentRange);
						return {
			...
			},
			{
				...
				value: function _makeXHRRequest(method, range, callbacks) {
					...
					var onXHRLoad = function onXHRLoad() {
						if (xhr.status === 200 || xhr.status === 206) callbacks.onSuccess(xhr);
						else if (callbacks.onError) callbacks.onError({
							"type": "xhr",
					...
					if (typeof xhr.onload !== "undefined") {
						...
						if (xhr.readyState === 4) onXHRLoad();
					};
					if (XhrFileReader._config.timeoutInSec) {
						xhr.timeout = XhrFileReader._config.timeoutInSec * 1e3;
					...
					if (range) this._setRequestHeader(xhr, "Range", "bytes=" + range[0] + "-" + range[1]);
			...
			},
			{
				...
				value: function _setRequestHeader(xhr, headerName, headerValue) {
					if (XhrFileReader._config.disallowedXhrHeaders.indexOf(headerName.toLowerCase()) < 0) xhr.setRequestHeader(headerName, headerValue);
				}
			},
			{
				...
				value: function _hasResponseHeader(xhr, headerName) {
					...
					if (!allResponseHeaders) return false;
					var headers = allResponseHeaders.split("\r\n");
					var headerNames = [];
					for (var i = 0; i < headers.length; i++) headerNames[i] = headers[i].split(":")[0].toLowerCase();
			...
			},
			{
				...
				value: function _getResponseHeader(xhr, headerName) {
					if (!this._hasResponseHeader(xhr, headerName)) return null;
			...
			},
			{
				key: "getByteAt",
				value: function getByteAt(offset) {
					return this._fileData.getByteAt(offset).charCodeAt(0) & 255;
				}
			},
			{
				key: "_isWebWorker",
			...
			},
			{
				...
				value: function _createXHRObject() {
					if (typeof window === "undefined" && !this._isWebWorker()) return new (require_browser()).XMLHttpRequest();
			...
			value: function setConfig(config) {
				for (var key in config) if (config.hasOwnProperty(key)) this._config[key] = config[key];
				var disallowedXhrHeaders = this._config.disallowedXhrHeaders;
				for (var i = 0; i < disallowedXhrHeaders.length; i++) disallowedXhrHeaders[i] = disallowedXhrHeaders[i].toLowerCase();
			}
		}]);
...
//#endregion
//#region node_modules/jsmediatags/build2/BlobFileReader.js
var require_BlobFileReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(obj) {
		"@babel/helpers - typeof";
		if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function _typeof(obj) {
			return typeof obj;
	...
	var ChunkedFileData = require_ChunkedFileData();
	module.exports = /* @__PURE__ */ function(_MediaFileReader) {
		...
		_createClass(BlobFileReader, [
			...
			},
			{
				...
				value: function loadRange(range, callbacks) {
					var self = this;
					var blob = (this._blob.slice || this._blob.mozSlice || this._blob.webkitSlice).call(this._blob, range[0], range[1] + 1);
	...
	}(require_MediaFileReader());
}));
//#endregion
//#region node_modules/jsmediatags/build2/ArrayFileReader.js
var require_ArrayFileReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(obj) {
		"@babel/helpers - typeof";
		if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function _typeof(obj) {
			return typeof obj;
	...
	module.exports = /* @__PURE__ */ function(_MediaFileReader) {
		...
		_createClass(ArrayFileReader, [
			...
			},
			{
				key: "getByteAt",
				value: function getByteAt(offset) {
					if (offset >= this._array.length) throw new Error("Offset " + offset + " hasn't been loaded yet.");
...
//#endregion
//#region node_modules/jsmediatags/build2/MediaTagReader.js
var require_MediaTagReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _classCallCheck(instance, Constructor) {
		if (!(instance instanceof Constructor)) throw new TypeError("Cannot call a class as a function");
	}
	function _defineProperties(target, props) {
		...
		_createClass(MediaTagReader, [
			...
			},
			{
				...
				value: function _expandShortcutTags(tagsWithShortcuts) {
					if (!tagsWithShortcuts) return null;
					var tags = [];
					var shortcuts = this.getShortcuts();
					for (var i = 0, tagOrShortcut; tagOrShortcut = tagsWithShortcuts[i]; i++) tags = tags.concat(shortcuts[tagOrShortcut] || [tagOrShortcut]);
					return tags;
				}
			}
		], [{
...
//#endregion
//#region node_modules/jsmediatags/build2/ID3v1TagReader.js
var require_ID3v1TagReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(obj) {
		"@babel/helpers - typeof";
		if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function _typeof(obj) {
			return typeof obj;
	...
	var ID3v1TagReader = /* @__PURE__ */ function(_MediaTagReader) {
		...
		_createClass(ID3v1TagReader, [{
			...
			value: function _parseData(data, tags) {
				...
				if (genreIdx < 255) var genre = GENRES[genreIdx];
				else var genre = "";
				var tag = {
					"type": "ID3",
					"version": version,
			...
			key: "canReadTagFormat",
			value: function canReadTagFormat(tagIdentifier) {
				return String.fromCharCode.apply(String, tagIdentifier.slice(0, 3)) === "TAG";
			}
		}]);
...
//#endregion
//#region node_modules/jsmediatags/build2/ID3v2FrameReader.js
var require_ID3v2FrameReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _classCallCheck(instance, Constructor) {
		if (!(instance instanceof Constructor)) throw new TypeError("Cannot call a class as a function");
	}
	function _defineProperties(target, props) {
		...
		_createClass(ID3v2FrameReader, null, [
			{
				...
				value: function getFrameReaderFunction(frameId) {
					if (frameId in frameReaderFunctions) return frameReaderFunctions[frameId];
					else if (frameId[0] === "T") return frameReaderFunctions["T*"];
					else if (frameId[0] === "W") return frameReaderFunctions["W*"];
					else return null;
				}
			},
			{
				...
				value: function readFrames(offset, end, data, id3header, tags) {
					...
					while (offset < end - frameHeaderSize) {
						...
						if (tags && tags.indexOf(frameId) === -1) continue;
						if (frameId === "MP3e" || frameId === "\0MP3" || frameId === "\0\0MP" || frameId === " MP3") break;
			...
			},
			{
				...
				value: function _getFrameHeaderSize(id3header) {
					...
					if (major == 2) return 6;
					else if (major == 3 || major == 4) return 10;
					else return 0;
				}
			},
			{
				...
				value: function _readFrameHeader(data, offset, id3header) {
					...
					if (frameId == String.fromCharCode(0, 0, 0) || frameId == String.fromCharCode(0, 0, 0, 0)) frameId = "";
					if (frameId) {
						if (major > 2) flags = this._readFrameFlags(data, offset + 8);
					}
					return {
						"id": frameId || "",
						"size": frameSize || 0,
			...
			},
			{
				...
				value: function _getFrameDescription(frameId) {
					if (frameId in FRAME_DESCRIPTIONS) return FRAME_DESCRIPTIONS[frameId];
					else return "Unknown";
				}
			},
			{
				...
				value: function getUnsyncFileReader(data, offset, size) {
					...
					for (var i = 0; i < frameData.length - 1; i++) if (frameData[i] === 255 && frameData[i + 1] === 0) frameData.splice(i + 1, 1);
	...
	frameReaderFunctions["APIC"] = function readPictureFrame(offset, length, data, flags, id3header) {
		...
		switch (id3header && id3header.major) {
			...
			case 4:
				...
				break;
			default: throw new Error("Couldn't read ID3v2 major version.");
		}
		var type = PICTURE_TYPE[data.getByteAt(offset)];
	...
	frameReaderFunctions["TXXX"] = function readTextFrame(offset, length, data, flags, id3header) {
		return getUserDefinedFields(offset, length, data, getTextEncoding(data.getByteAt(offset)));
	};
	...
	frameReaderFunctions["TCON"] = function readGenreFrame(offset, length, data, flags) {
		return frameReaderFunctions["T*"].apply(this, arguments).replace(/^\(\d+\)/, "");
	};
...
//#endregion
//#region node_modules/jsmediatags/build2/ID3v2TagReader.js
var require_ID3v2TagReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(obj) {
		"@babel/helpers - typeof";
		if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function _typeof(obj) {
			return typeof obj;
		...
		_createClass(ID3v2TagReader, [
			...
			},
			{
				...
				value: function _parseData(data, tags) {
					...
					if (tags) var expandedTags = this._expandShortcutTags(tags);
					...
					for (var name in SHORTCUTS) if (SHORTCUTS.hasOwnProperty(name)) {
						...
						if (frameData) id3.tags[name] = frameData;
					}
					for (var frame in frames) if (frames.hasOwnProperty(frame)) id3.tags[frame] = frames[frame];
					return id3;
				}
			},
			{
				...
				value: function _getFrameData(frames, ids) {
					var frame;
					for (var i = 0, id; id = ids[i]; i++) if (id in frames) {
						if (frames[id] instanceof Array) frame = frames[id][0];
						else frame = frames[id];
						return frame.data;
					}
				}
			},
			...
			key: "canReadTagFormat",
			value: function canReadTagFormat(tagIdentifier) {
				return String.fromCharCode.apply(String, tagIdentifier.slice(0, 3)) === "ID3";
			}
		}]);
...
//#endregion
//#region node_modules/jsmediatags/build2/MP4TagReader.js
/**
...
* 
*/
var require_MP4TagReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(obj) {
		"@babel/helpers - typeof";
		if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function _typeof(obj) {
			return typeof obj;
		...
		_createClass(MP4TagReader, [
			...
			},
			{
				...
				value: function _loadAtom(mediaFileReader, offset, parentAtomFullName, callbacks) {
					...
					if (this._isContainerAtom(atomName)) {
						if (atomName == "meta") offset += 4;
						var atomFullName = (parentAtomFullName ? parentAtomFullName + "." : "") + atomName;
						if (atomFullName === "moov.udta.meta.ilst") mediaFileReader.loadRange([offset, offset + atomSize], callbacks);
						else mediaFileReader.loadRange([offset + 8, offset + 8 + 8], {
							onSuccess: function onSuccess() {
								self._loadAtom(mediaFileReader, offset + 8, atomFullName, callbacks);
							},
							onError: callbacks.onError
						});
					} else mediaFileReader.loadRange([offset + atomSize, offset + atomSize + 8], {
						onSuccess: function onSuccess() {
			...
			},
			{
				...
				value: function _parseData(data, tagsToRead) {
					...
					for (var name in SHORTCUTS) if (SHORTCUTS.hasOwnProperty(name)) {
						...
						if (tag) if (name === "track") tags[name] = tag.data.track;
						else tags[name] = tag.data;
					}
					return {
						"type": "MP4",
			...
			},
			{
				...
				value: function _readAtom(tags, data, offset, length, tagsToRead, parentAtomFullName, indent) {
					...
					while (seek < offset + length) {
						...
						if (this._isContainerAtom(atomName)) {
							if (atomName == "meta") seek += 4;
							var atomFullName = (parentAtomFullName ? parentAtomFullName + "." : "") + atomName;
							this._readAtom(tags, data, seek + 8, atomSize - 8, tagsToRead, atomFullName, indent);
							return;
						}
						if ((!tagsToRead || tagsToRead.indexOf(atomName) >= 0) && parentAtomFullName === "moov.udta.meta.ilst" && this._canReadAtom(atomName)) tags[atomName] = this._readMetadataAtom(data, seek);
						seek += atomSize;
					}
				}
			},
			{
				...
				value: function _readMetadataAtom(data, offset) {
					...
					var type = TYPES[data.getInteger24At(offset + METADATA_HEADER + 1, true)];
					var atomData;
					var bigEndian = true;
					if (atomName == "trkn") atomData = {
						"track": data.getShortAt(offset + METADATA_HEADER + 10, bigEndian),
						"total": data.getShortAt(offset + METADATA_HEADER + 14, bigEndian)
					};
					else if (atomName == "disk") atomData = {
						...
						var atomData;
						if (atomName === "covr" && type === "uint8") type = "jpeg";
						switch (type) {
							case "text":
							...
							case "int":
							case "uint":
								atomData = (type == "int" ? dataLength == 1 ? data.getSByteAt : dataLength == 2 ? data.getSShortAt : dataLength == 4 ? data.getSLongAt : data.getLongAt : dataLength == 1 ? data.getByteAt : dataLength == 2 ? data.getShortAt : data.getLongAt).call(data, dataStart + (dataLength == 8 ? 4 : 0), true);
								break;
							case "jpeg":
							case "png":
								atomData = {
			...
			key: "canReadTagFormat",
			value: function canReadTagFormat(tagIdentifier) {
				return String.fromCharCode.apply(String, tagIdentifier.slice(4, 8)) === "ftyp";
			}
		}]);
		return MP4TagReader;
	}(MediaTagReader);
...
//#endregion
//#region node_modules/jsmediatags/build2/FLACTagReader.js
var require_FLACTagReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(obj) {
		"@babel/helpers - typeof";
		if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function _typeof(obj) {
			return typeof obj;
	...
	module.exports = /* @__PURE__ */ function(_MediaTagReader) {
		...
		_createClass(FLACTagReader, [
			{
				...
				value: function _loadData(mediaFileReader, callbacks) {
					var self = this;
					mediaFileReader.loadRange([4, 7], { onSuccess: function onSuccess() {
			...
			},
			{
				...
				value: function _loadBlock(mediaFileReader, offset, callbacks) {
					...
					if (COMMENT_HEADERS.indexOf(blockHeader) !== -1) {
						...
						mediaFileReader.loadRange([offsetMetadata, offsetMetadata + blockSize], { onSuccess: function onSuccess() {
					...
					} else self._nextBlock(mediaFileReader, offset, blockHeader, blockSize, callbacks);
				}
			},
			{
				key: "_nextBlock",
				value: function _nextBlock(mediaFileReader, offset, blockHeader, blockSize, callbacks) {
					var self = this;
					if (blockHeader > 127) if (!self._commentOffset) callbacks.onError({
						"type": "loadData",
						"info": "Comment block could not be found."
					});
					else callbacks.onSuccess();
					else mediaFileReader.loadRange([offset + 4 + blockSize, offset + 4 + 4 + blockSize], { onSuccess: function onSuccess() {
			...
			},
			{
				key: "_parseData",
				value: function _parseData(data, tags) {
					var offsetList = data.getLongAt(this._commentOffset, false) + (this._commentOffset + 4);
			...
			key: "canReadTagFormat",
			value: function canReadTagFormat(tagIdentifier) {
				return String.fromCharCode.apply(String, tagIdentifier.slice(0, 4)) === "fLaC";
			}
		}]);
...
//#endregion
//#region node_modules/jsmediatags/build2/ReactNativeFileReader.js
var require_ReactNativeFileReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(obj) {
		"@babel/helpers - typeof";
		if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function _typeof(obj) {
			return typeof obj;
		...
		_createClass(ReactNativeFileReader, [
			...
			},
			{
				...
				value: function loadRange(range, callbacks) {
					...
					RNFS.read(this._path, length, range[0], { encoding: "base64" }).then(function(readData) {
...
//#endregion
//#region node_modules/jsmediatags/build2/NodeFileReader.js
var require_NodeFileReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(obj) {
		"@babel/helpers - typeof";
		if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function _typeof(obj) {
			return typeof obj;
...
//#endregion
//#region node_modules/jsmediatags/build2/jsmediatags.js
var require_jsmediatags = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _classCallCheck(instance, Constructor) {
		if (!(instance instanceof Constructor)) throw new TypeError("Cannot call a class as a function");
	}
	function _defineProperties(target, props) {
	...
	var Reader = /* @__PURE__ */ function() {
		...
		_createClass(Reader, [
			...
			},
			{
				key: "read",
				value: function read(callbacks) {
					var fileReader = new (this._getFileReader())(this._file);
					var self = this;
					fileReader.init({
						onSuccess: function onSuccess() {
			...
			},
			{
				...
				value: function _getFileReader() {
					...
					else return this._findFileReader();
				}
			},
			{
				key: "_findFileReader",
				value: function _findFileReader() {
					for (var i = 0; i < mediaFileReaders.length; i++) if (mediaFileReaders[i].canReadFile(this._file)) return mediaFileReaders[i];
			...
			},
			{
				...
				value: function _getTagReader(fileReader, callbacks) {
					...
					} else this._findTagReader(fileReader, callbacks);
				}
			},
			{
				key: "_findTagReader",
				value: function _findTagReader(fileReader, callbacks) {
					...
					for (var i = 0; i < mediaTagReaders.length; i++) {
						...
						if (range.offset >= 0 && range.offset < fileSize / 2 || range.offset < 0 && range.offset < -fileSize / 2) tagReadersAtFileStart.push(mediaTagReaders[i]);
						else tagReadersAtFileEnd.push(mediaTagReaders[i]);
					}
		...
		_createClass(Config, null, [
			...
			},
			{
				...
				value: function removeTagReader(tagReader) {
					...
					if (tagReaderIx >= 0) mediaTagReaders.splice(tagReaderIx, 1);
					return Config;
				}
			},
			{
	...
	if (typeof process !== "undefined" && !process.browser) if (typeof navigator !== "undefined" && navigator.product === "ReactNative") {
		var ReactNativeFileReader = require_ReactNativeFileReader();
		Config.addFileReader(ReactNativeFileReader);
	} else {
		var NodeFileReader = require_NodeFileReader();
...
//#endregion
export default require_jsmediatags();
